...
procedure MixedScheduling is
   ...
   Start : Time; -- Start Time of the System
	Calibrator: constant Integer := 1500; -- Calibration for correct timing
   ...
   begin
      loop
         -- Simulate 100 ms workload
         for I in 1..100 loop
            Dummy := F(Calibrator);
         end loop;
         Put_Line("Background Task " & Integer'Image(Id) & " executed at " & Duration'Image(To_Duration(Clock - Start)));
      end loop;
   end B;
